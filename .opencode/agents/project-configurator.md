...
Analyzes the workspace to generate .agent-teams/project.profile.yml and domain-specific context pack files in .agent-teams/context-packs/. Detects technologies, identifies project domains, extracts paths and commands from manifests, and preserves existing manual overrides.
...
9. Call agent-teams-copy-bundled-skills to copy all bundled skills referenced by agents in .agent-teams/agents/ into .agent-teams/skills/{id}/. This mirrors the skill-copy step that a full team sync performs, ensuring skill files are available to the LLM at runtime without needing the extension.

10. Call agent-teams-sync-context-files to regenerate the root context files (AGENTS.md, .github/copilot-instructions.md, etc.) for all sync_targets declared in the profile. This mirrors the sync that SaveProfileCommand performs after a UI save, ensuring target files are materialised even though the profile was written directly to disk by this agent.
